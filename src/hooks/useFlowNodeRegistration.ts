...
export function useFlowNodeRegistration({
  ...
  const createTransactionRecord = useCallback(async (draft: TransactionRecordDraft) => {
    if (!selectedLocalNode) return
    const consumeNode = localConsumeNodes.find((node) => node.publicKeyHex === draft.consumeNodePubkey)
    if (!consumeNode) {
      dispatch({ type: 'FAILURE', error: 'Add or pick a consume node first.' })
      return
    }
    ...
    try {
      ...
      setRecordFormOpen(false)
      dispatch({ type: 'SUCCESS', status: `Transaction record created (${shortId(record.id)}).` })
    } catch (operationError) {
      dispatch({
        type: 'FAILURE',
        error: errorMessage(operationError, 'Failed to create transaction record'),
      })
    } finally {
  ...
  const createTransactionMount = useCallback(async (recordId: string, difficultyHex: string) => {
    ...
    if (!consumeNode || !flowNode) {
      dispatch({ type: 'FAILURE', error: 'The consume/flow node for this record is not in your keyring.' })
      return
    }
    ...
    try {
      ...
      dispatch({ type: 'SUCCESS', status: 'Transaction mounted. View the consume chain to see it on the graph.' })
    } catch (operationError) {
      dispatch({
        type: 'FAILURE',
        error: errorMessage(operationError, 'Failed to mount transaction'),
      })
    } finally {
      dispatch({ type: 'SET_BUSY', busy: null })
      setMiningAttempts(null)
  ...
  const fetchRegisterDifficulty = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SUCCESS', status: `Latest register difficulty loaded from block ${block.height ?? '-'}.` })
    } catch (operationError) {
      dispatch({
        type: 'FAILURE',
        error: errorMessage(operationError, 'Failed to load latest block'),
      })
    } finally {
      dispatch({ type: 'SET_BUSY', busy: null })
    }
  ...
  const registerFlowNode = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: 'SUCCESS',
        status: `Registered ${shortId(selectedLocalNode.publicKeyHex)} with nonce ${built.nonce}.`,
      })
  ...
  const authorizeCentralPubkey = useCallback(async () => {
    ...
    try {
      ...
      setLastRawBytes(built.rawBytesHex)
      dispatch({ type: 'SUCCESS', status: `Authorized central pubkey ${shortId(normalizedCentralPubkey)}.` })
      void reloadLocalNodeState(selectedLocalNode.publicKeyHex)
    } catch (operationError) {
